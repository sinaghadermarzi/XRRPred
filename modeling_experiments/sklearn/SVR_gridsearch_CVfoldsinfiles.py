...
def optimize_params_SVR(train_fold_dfs,test_fold_dfs,input_features,target_col,eval_func,tolerance,maximize,param_vals):
    ...
    for (gamma, C,criterion) in itertools.product(gammas,Cs,kernels):
...
for target_var in ["resolution","rfree"]:
    ...
    for sampling in sampling_methods:
        ...
        gamma,C, kernel = optimize_params_SVR(train_fold_norm_df,test_fold_norm_df,input_features,target_col,SPC,0.005,True,param_pool)
    ...
    results_df.to_csv("results_SVR_"+target_col+".csv",columns=sorted(results_df.columns),index = row_labels)
